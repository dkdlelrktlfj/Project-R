...
public class BSPMapGenerator
{
    ...
    private void DivideBinaryTree(RoomNode _rootNode, int _n, MapData _mapData)
    {
        if(_n < _mapData.maxNode) // 노드의 최댓값이 될 때 까지 반복
        {
            ...
            RectInt size = _rootNode.roomSize;
            ...
            else
            {
                ...
                _rootNode.leftLeaf = new RoomNode(size.x, size.y * split, size.width, size.height - split);
    ...
    public void GenerateDungeon(RoomNode _rootNode, int _n, MapData _mapdata)
    {
        if (_n == _mapdata.maxNode)
        {
            RectInt size = _rootNode.treeSize;

            int width = Mathf.Max(UnityEngine.Random.Range(size.width / 2, size.width - 1));
            int height = Mathf.Max(UnityEngine.Random.Range(size.height / 2, size.height - 1));
        }
    }
}